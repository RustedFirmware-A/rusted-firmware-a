...
impl<PlatformImpl: CpuStateAccess + PlatformErrata> Service for ErrataManagement<PlatformImpl> {
    ...
    fn handle_non_secure_smc(&self, regs: &mut SmcReturn) -> World {
        ...
        match function.0 {
            ...
            EM_CPU_ERRATUM_FEATURES => regs.set_from(cpu_erratum_features::<PlatformImpl>(in_regs)),
...
mod tests {
    ...
    fn em_version_non_secure() {
        ...
        assert_eq!(
            ErrataManagement::<TestPlatform>::new().handle_non_secure_smc(&mut regs),
            World::NonSecure
        );
    ...
    fn em_version_secure_not_supported() {
        ...
        regs.mark_all_used()[0] = EM_VERSION.into();
        assert_eq!(
            ErrataManagement::<TestPlatform>::new().handle_secure_smc(&mut regs),
            World::Secure
        );
